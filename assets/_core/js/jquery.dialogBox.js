 ...
 ;(function($,window,document){
 	...
 	var pluginName = 'dialogBox',
 		defaults = {
 			...
 			hasBtn: false,  //boolean,是否显示操作按钮，如Cancel，Confirm
 			confirmValue: null,  //string,Confirm按钮文字内容
 			confirm: function(){}, //function,点击Confirm后回调函数
 			cancelValue: null,  //string,Cancel按钮文字内容
 			cancel: function(){},  //function,点击Cancel后回调函数，默认关闭弹出框
 		...
 		create: function(element){
 			...
 			if(!hasBtn){
 				...
 				if(confirmValue && cancelValue){
 					...
 					dialogHTML[1] = '<div class="dialog-btn"><span class="dialog-btn-cancel">Cancel</span><span class="dialog-btn-confirm">Confirm</span></div></div></section>';
 				}
 			}
 		...
 		trigger: function(element,event){
 			...
 			if($.isFunction(that.settings.callback)){
 				that.settings.callback();
                        ...
                        //,#dialog-box-mask
 			$('.dialog-box-close').on('click',function(event){
				that.settings.close.call(that);
				event.stopPropagation();
			})
                        //,#dialog-box-mask
 			$('.dialog-box-close,.dialog-btn-cancel,.dialog-btn-confirm').on('click',function(event){
 				that.hide($this);
 				event.stopPropagation();
 			});
