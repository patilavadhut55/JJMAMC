...
//				$this->objParentControl->blnModified = true;
		...
		public function RenderWithError($blnDisplayOutput = true) {
			...
			try {
				...
				if ($this->strValidationError)
					$strOutput .= sprintf('<br %s/><p %sclass="text-danger">%s</span>', $strDataRel, $strDataRel, $this->strValidationError);
				else if ($this->strWarning)
					$strOutput .= sprintf('<br %s/><p %sclass="text-danger">%s</span>', $strDataRel, $strDataRel, $this->strWarning);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
			}
