...
/*				$strControlId = $objParentObject->ControlId;
		...
		public function RenderBegin($blnDisplayOutput = true) {
			...
			// Setup Rendered HTML
			$strToReturn .= sprintf('<form method="post" id="%s" enctype="multipart/form-data" action="%s"%s >', $this->strFormId, htmlentities(QApplication::$RequestUri), $strFormAttributes);
			$strToReturn .= "\r\n";
		...
		public function RenderEnd($blnDisplayOutput = true) {
			...
			$strJavaScriptArray = $this->ProcessJavaScriptList(__JQUERY_BASE__ . ', ' . __JQUERY_EFFECTS__ . ',jquery/jquery.ajaxq-0.0.1.js,qcubed.js');
			// Setup IncludeJs
			$strToReturn = "\r\n";
