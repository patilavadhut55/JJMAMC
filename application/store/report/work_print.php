...
class PO extends QForm {
    ...
    protected function chkApprove_click() {
        ...
        if ($this->chkApprove->Checked == 1)
            ...
            $status = "Rejected";
        QApplication::DisplayAlert("Work Order for " . $vov->Narration . ' To supplier' . $vov->ToObject . ' has ' . $status);
    }
    protected function chkhead_Click() {
        ...
        if($this->chkhead->Checked == TRUE){
            $flag = 1;
        QApplication::Redirect(__VIRTUAL_DIRECTORY__ . __FORM_APPLICATION__ . '/store/report/work_print.php?id='.$_GET['id'].'&flag=1');
                    
        }
        else{
            $flag = 0;
              QApplication::Redirect(__VIRTUAL_DIRECTORY__ . __FORM_APPLICATION__ . '/store/report/work_print.php?id='.$_GET['id'].'&flag=0');
