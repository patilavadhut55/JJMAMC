<?php
	...
	class AdmittedReport extends QForm {
        ...
        $this->lstDist->SelectionMode = QSelectionMode::Multiple;
        $this->lstDist->Height = 100;
        
        $district = Place::LoadArrayByParrent(2);
        if ($district) {
            foreach ($district as $districts) {
                $this->lstDist->AddItem($districts->Name, $districts->Idplace);
            }
        }
...
//                    }            
//                    
        /*if($this->lstDist->SelectedValues ){
                         $districts = Place::LoadByIdplace($this->lstDist->SelectedValues);
            ...
            protected function Report(){
                if($this->lstYear->SelectedValue != NULL && $this->lstCourse->SelectedValue != NULL){
                    //QApplication::Redirect(__VIRTUAL_DIRECTORY__.__FORM_APPLICATION__.'/admission/report1/distwise_report.php?dept='.$this->lstCourse->SelectedValue.'&cal='.$this->lstYear->SelectedValue .'&dist='.$this->lstDist->SelectedValue);
                }else{
