...
//		protected function Form_Load() {}
		...
		protected function Form_Create() {
                    ...
                    $this->lstPaymentMode = new QListBox($this);
                    $this->lstPaymentMode->Name = "Payment Mode";
                    $this->lstPaymentMode->AddItem("-Select All-",NULL);
                    $pays = PaymentMode::LoadAll();
                    foreach ($pays as $pay){
                        $this->lstPaymentMode->AddItem($pay->Name,$pay->IdpaymentMode);
                    }
                    
                    if(isset($_GET['paymode'])){
                        $this->lstPaymentMode->SelectedValue = $_GET['paymode'];
                    }
                ...
                protected function Report(){
                    ...
                    if($this->calfrom->Text != ""){
                        if($param == ""){
                            $param = "?from=".date("Ymd",  strtotime($this->calfrom->Text));
                        }                        
                    }
                    
                    if($this->calto->Text != ""){
                        if($param == ""){
                            $param = "?to=".date("Ymd",  strtotime($this->calto->Text));
                        }else{
                            $param .= "&to=".date("Ymd",  strtotime($this->calto->Text));
                        }
                    }
                    ...
                    if($this->lstYear->SelectedValue != NULL){
                        if($param == ""){
                            $param = "?acdyear=".$this->lstYear->SelectedValue;
                        }else{
                            $param .= "&acdyear=".$this->lstYear->SelectedValue;
                        }
                    }
                    ...
                    if($this->lstSemister->SelectedValue != NULL){
                       if($param == ""){
                            $param = "?semi=".$this->lstSemister->SelectedValue;
                        }else{
                            $param .= "&semi=".$this->lstSemister->SelectedValue;
                        }
                    }
                    
                    if($this->lstadmission->SelectedValue != NULL){
                        if($param == ""){
                            $param = "?addyear=".$this->lstadmission->SelectedValue;
                        }else{
                            $param .= "&addyear=".$this->lstadmission->SelectedValue;
                        }                       
                    }
                    
                    if($this->lstPaymentMode->SelectedValue != ""){
                        if($param == ""){
                            $param = "?paymode=".$this->lstPaymentMode->SelectedValue;
                        }else{
                            $param .= "&paymode=".$this->lstPaymentMode->SelectedValue;
                        }  
                    }
                ...
                QApplication::Redirect(__VIRTUAL_DIRECTORY__.__FORM_APPLICATION__.'/report/daily_fee_report.php'.$param);                   
