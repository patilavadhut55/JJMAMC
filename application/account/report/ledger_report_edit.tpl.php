...
?>
<div id="formPrint">
<h2><?php if(isset($_GET["ledger"])) ?>
 <?php if($ledger->GrpObject->Grp == 23 || $ledger->GrpObject->Grp == 27){_p($ledger->Name);?>'s <?php }else{_p($ledger->Name);}?> Ledger Report </h2>
...
<tr>
    ...
    $voutures = Voucher::QueryArray(
            QQ::AndCondition(
                QQ::GreaterOrEqual(QQN::Voucher()->Date , date ('Ymd',  strtotime($Opy."0401")).'000000'),
                QQ::LessOrEqual(QQN::Voucher()->Date , date ('Ymd',  strtotime ($from)).'235959'),
...
<?php 
    $voutures = Voucher::QueryArray(
            QQ::AndCondition(
                QQ::GreaterOrEqual(QQN::Voucher()->Date , date ('Ymd',  strtotime ($from)).'000000'),
                QQ::LessOrEqual(QQN::Voucher()->Date , date ('Ymd',  strtotime ($to)).'235959'),
                QQ::OrCondition(
                    QQ::Equal(QQN::Voucher()->Dr , $_GET["ledger"]),
                    QQ::Equal(QQN::Voucher()->Cr , $_GET["ledger"])
                )
...
<a href="<?php _p(__VIRTUAL_DIRECTORY__.__FORM_APPLICATION__); ?>/vouture/voucher_edit.php?id=<?php _p($ref->Idvoucher); ?>" target="_blank">
...
<?php } ?>
    ...
    <?php  
        if($vouture->Narration != NULL){ 
            _P('<Br/>'.base64_decode($vouture->Narration), FALSE);
        }else{
