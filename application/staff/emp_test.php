   ...
   // protected $txtPassYear;
    ...
    protected function Form_Create() {
      ...
      //  $this->calAdmitdate->Text = date('d/m/Y',  strtotime(QDateTime::Now()));
       // $this->calAdmitdate->DateTimeFormat = 'dd/mm/yy';
        ...
        $this->lststate->AddAction(new QChangeEvent(), new QAjaxAction('lststate_Change'));
        ...
        $this->txtCity->Width = $this->txtCity1->Width = $this->lstdistrict->Width = $this->lstdistrict1->Width = "7x0%";
      ...
      //  $this->txtNote->AddAction(new QEnterKeyEvent(), new QServerAction('txtNote_EnterKey'));
        ...
        if(isset($_GET['id'])){
            ...
            $memdocs = MemberDoc::LoadArrayByMember($employee->Of);
            if($memdocs){
    ...
    protected function btnAdmissionSave_click(){
        ...
        if($error == 0) {
            if($this->txtempfname->Text != "" && $this->txtempmname->Text != "" && $this->txtemplname->Text != ""  && $this->calAdmitdate->Text != "" ) {
                ...
                QApplication::Redirect(__VIRTUAL_DIRECTORY__ . __FORM_APPLICATION__ . '/staff/emp_test.php?id='.$employee->Idaddress.'&tab=2');
            }
            else 
                {
    ...
    protected function btnPersonalSave_click(){
        if(isset($_GET['id'])) {
            ...
            $employee->Save();
            QApplication::Redirect(__VIRTUAL_DIRECTORY__ . __FORM_APPLICATION__ . '/staff/emp_test.php?id='.$_GET['id'].'&tab=3');
        }
      ...
      protected function btninfosave_click(){
        if(isset($_GET['id'])) {
            ...
            $employee->PanNo = $this->txtpanno->Text;
               $employee->Save();
            QApplication::Redirect(__VIRTUAL_DIRECTORY__ . __FORM_APPLICATION__ . '/staff/emp_test.php?id='.$_GET['id'].'&tab=4');
        }
    }
       ...
       public function btnEduDetailsSave_Click($strFormId, $strControlId, $strParameter){
        if(isset($_GET['id'])){
            ...
            if($this->txttitle->SelectedValue != NULL){
                ...
                $stud = EducationDetails::LoadArrayByStudent($employee->Of);
                ...
                $education->Save();
                QApplication::Redirect(__VIRTUAL_DIRECTORY__.__FORM_APPLICATION__. '/staff/emp_test.php?id='.$_GET['id'].'&tab=5'); 
  ...
  protected function lstpayscale_Change() {
        if ($this->lstpayscale->SelectedValue != NULL) {
            $payscale = Address::QueryArray(
                            QQ::AndCondition(
                                QQ::Equal(QQN::Address()->Parrent, $this->lstpayscale->SelectedValue)
                            ));
            foreach ($places as $place) {
    ...
    protected function txtNote_EnterKey(){
                    ...
                    $exper->Save();
                   QApplication::Redirect(__VIRTUAL_DIRECTORY__.__FORM_APPLICATION__.'/staff/emp_test.php?id='.$_GET['id'].'#Experience');   
                    }else {
    ...
    protected function btnexpsave_Click(){
        if(isset($_GET['id']) && $this->calRetiredate->Text != "" && $this->calleftdate->Text != "" ){
            ...
            QApplication::Redirect(__VIRTUAL_DIRECTORY__.__FORM_APPLICATION__.'/staff/emp_test.php?id='.$_GET['id'].'#Document');
        }
    }
  ...
  protected function btnAdd_Click(){
        if($this->lstDocs->SelectedValue != NULL && isset($_GET['id'])){
            ...
            if($employee){
                ...
                $memdoc->Member = $employee->Of;
                $memdoc->Document = $this->lstDocs->SelectedValue;
                $memdoc->Required = TRUE;
                $memdoc->Save();
            QApplication::Redirect(__VIRTUAL_DIRECTORY__.__FORM_APPLICATION__.'/staff/emp_test.php?id='.$_GET['id'].'&docid='.$memdoc->IdmemberDoc.'&code='.$scancode->Option.'&doc='.$scancode->Option.'&tab=6');
            }
        }else{
    ...
    protected function btnUpload_Click($strFormId, $strControlId, $strParameter){
        if(isset($_GET['id'])){
            $file_name = $_FILES['imgupload']['name'];
                ...
                $app = Address::LoadByIdaddress($_GET['id']);
                //$app = DeptYearEvents::LoadByIddeptYearEvents($_GET['id']);
                $appdocs = MemberDoc::LoadArrayByMeeting($_GET['id']);
                foreach ($appdocs as $appdoc){} 
                    $target_file = "../upload/documents/A".$app->OfObject->Code."/".$_GET['doc'].'.'.$file_ext;
                    $files = glob('../upload/documents/A'.$app->OfObject->Code."/*.*");
    ...
    protected function lbldocdelete_click($strFormId, $strControlId, $strParameter){
        ...
        $memdoc->Delete();
        QApplication::Redirect(__VIRTUAL_DIRECTORY__.__FORM_APPLICATION__.'/staff/emp_test.php?id='.$_GET['id'].'&tab=6');
    } 
}
