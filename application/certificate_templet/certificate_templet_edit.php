<?php
	...
	class CertificateTempletEditForm extends CertificateTempletEditFormBase {
		// Override Form Event Handlers as Needed
                protected $dtgCast;
                protected $btnList;
                protected $btnNew;
                protected $lstCategory;
                //protected $lstleaveCategory;
...
//		protected function Form_Load() {}
		...
		protected function Form_Create() {
                    ...
                    $grps = CertificateGroup::QueryArray(
                                QQ::AndCondition(
                                    QQ::Equal(QQN::CertificateGroup()->IdcertificateGroup,4)
                                )
                            );
                    foreach ($grps as $grp) {
                    $this->lstgroup->AddItem($grp->Name, $grp->IdcertificateGroup);
                    }
                    $this->lstgroup->AddAction(new QClickEvent(), new QServerAction('search_change'));
                ...
                /*    $this->lstleaveCategory = new QSelect2ListBox($this);                    
                    $this->lstleaveCategory->Name = "Leave Category"; 
                    $this->lstleaveCategory->AddItem("-Select-",NULL);
                    $leavecats = LeaveCat::LoadAll();
                    foreach ($leavecats as $leavecat){
                        $this->lstleaveCategory->AddItem($leavecat->Name,$leavecat->IdleaveCat);    
                    }
                 * */
                    ...
                    if($this->mctCertificateTemplet->EditMode == TRUE){
                       ...
                       // $this->lstleaveCategory->SelectedValue = $certemp->LeaveCat;
                    }else{
...
//                            } 
                ...
                protected function search_change($strFormId, $strControlId, $strParameter){
		...
		protected function btnSave_Click($strFormId, $strControlId, $strParameter) {
			// Delegate "Save" processing to the CertificateTempletMetaControl
                     //   $this->mctCertificateTemplet->LeaveCatControl->SelectedValue = $this->lstleaveCategory->SelectedValue;
                        //$this->txtCertificateHtml->Text = base64_encode($this->txtPrintCode->Text);
	...
	CertificateTempletEditForm::Run('CertificateTempletEditForm');
?>